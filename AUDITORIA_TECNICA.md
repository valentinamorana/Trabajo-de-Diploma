...
- [ ] **Sin tests directos para `BLL/Pedido.cs`.** Es la clase más crítica del sistema (cupo de suscripción, alta de pedido, la lógica anti-TOCTOU de la transacción) y no tiene ni un test unitario propio — `Tests/ComandoPedidoTests.cs` solo prueba el wrapper Command contra un `IPedidoService` mockeado, nunca la implementación real. A diferencia de `BLL.Cliente` (que ya tomaba `IClienteDAL` inyectado), **`BLL.Pedido` hardcodea `new DAL.Pedido()`, `new DAL.Cliente()`, `new DAL.Empleado()`, `new DAL.PlanSuscripcion()` y `new DAL.PedidoHistorial()` directo en el constructor — no hay forma de inyectar dobles de prueba tal como está hoy.** Escribirle tests reales requiere primero decidir si vale la pena refactorizarla para que reciba esas 5 dependencias por constructor (mismo patrón que ya tiene `BLL.Cliente`/`BLL.Renovacion`/`BLL.Cobro`) — es un cambio más grande y toca la clase más sensible de la app, así que no se hizo sin preguntar primero.
