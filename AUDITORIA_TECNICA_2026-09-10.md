...
**Hallazgo nuevo (2026-09-14, fuera de la numeración original) — ✅ RESUELTO — `BD/00_Instalacion_Completa.sql` (la "fuente única de verdad" según el propio README) nunca creaba la tabla `Usuario_Seguridad`.**
Verificación pedida explícitamente: "¿el SQL está up to date con el código?". Comparé cada `CREATE TABLE` de los 21 scripts de `BD/` contra `00_Instalacion_Completa.sql` — coincide todo salvo esta tabla. `DAL.EspejoUsuario` (T07, espejo de integridad) la lee/escribe activamente desde `DAL/Usuario.cs` en cada alta/modificación legítima (`Upsert` en dos sitios, `Reconstruir` en un tercero) y `BLL.RecuperacionIntegridad.Diagnosticar()` la usa para ofrecer "Reparar desde Espejo" en el diagnóstico de integridad — no es código muerto, es una feature activa. El problema: esa tabla solo se creaba en `02_Actualizar_BaseDeDatos.sql`, el script legacy para "migrar una BD MUY vieja (previa a 01)" — que el propio README dice que **no** hay que correr en una instalación nueva. Resultado: en toda instalación hecha por el camino documentado (`00` solo), la opción "Reparar desde Espejo" queda permanentemente no disponible en silencio (el código tolera la ausencia — no crashea — pero la feature nunca funciona, ni siquiera después de que la app la usa por un tiempo).
*Solución aplicada:* se agregó el bloque `CREATE TABLE Usuario_Seguridad` a `00_Instalacion_Completa.sql` (mismas columnas, mismo patrón `IF NOT EXISTS ... GO` que el resto del archivo), copiado del bloque ya probado en `02`. Resto del esquema (columnas agregadas por ALTER en los scripts incrementales, roles/usuarios demo de PN02/PN03) verificado presente en `00` — no se encontraron más gaps.
...
**4. ✅ RESUELTO (2026-09-14) — Detección de "BD sin migrar" por matcheo de texto sobre `SqlException.Message`.** Más de 10 veces en `DAL/Usuario.cs` (afecta el login), también en `DAL/Permiso.cs`/`DAL/VersionUsuario.cs`. Frágil: depende del idioma del servidor y de que el mensaje contenga la subcadena esperada; un error real que mencione esa palabra por coincidencia se trataría como "no migrado" en el camino más transitado del sistema.
*Solución aplicada:* en los 12 sitios de `DAL/Usuario.cs`+`DAL/VersionUsuario.cs` se reemplazó el `when (sqlEx.Message.Contains("NombreDeColumna") || ...)` por `when (sqlEx.Number == ColumnaInexistente)` (constante = 207, el código nativo de SQL Server para "Invalid column name" — independiente del idioma del servidor). Verificado antes de tocarlo que en TODOS los sitios el catch no distingue cuál columna específica falta — siempre cae al mismo fallback sin esa columna — así que el cambio es equivalente en comportamiento, solo más robusto. En `DAL/Permiso.cs` el problema era peor (`catch (SqlException)` sin ningún filtro, tragaba cualquier error de SQL como "no migrado"): ahora también filtra por `Number == 207`. *Sin cobertura de test directa* (un `SqlException` real no se puede instanciar desde código de aplicación — sus constructores son internos — así que esta rama solo se ejercita contra un SQL Server real sin migrar; se verificó por lectura de código, no por test).
...
**6. ✅ RESUELTO (2026-09-14) — Inconsistencia sistemática: `BaseDAL<T>` (que centraliza el acceso a BD) vs. ~17 clases DAL que redeclaran `Acceso.GetInstance()` a mano** en vez de heredar — pierden el contrato mínimo uniforme que la clase base fuerza.
*Solución aplicada:* de las 19 clases que no heredaban, solo 2 (`Empleado`, `PlanSuscripcion`) encajaban en el contrato completo `List<T> ObtenerTodos()`/`T ObtenerPorId(int)` de `BaseDAL<T>` — forzarlo en el resto (`Bitacora`/`BitacoraNegocio` devuelven `DataTable`, `Permiso` es un árbol, `Traduccion`/`Preferencia` se consultan por idioma/usuario no por ID, utilitarias como `DigitoVerificador`/`HistorialIntegridad`/`EspejoUsuario`/`PedidoHistorial`) hubiera sumado métodos sin uso real. Se separó `BaseDAL` en una base NO genérica (solo el campo `acceso` compartido) y `BaseDAL<T> : BaseDAL` (agrega el contrato de entidad); las 17 restantes heredan la base no genérica. Verificado antes de tocarlo que `BaseDAL<T>` nunca se usa polimórficamente en el repo, así que el cambio de jerarquía era seguro. Sin cambio de comportamiento.
...
**5. (Diferido, a propósito — no junto con §1 #4) `BLL/Configuracion.cs:220-222` (`VerificarIntegridadDV`) — heurística de texto frágil** (`msg.Contains("DVH")`) para distinguir "BD sin migrar" de corrupción real; un error real que mencione esas palabras por coincidencia se trataría como no-migrado. *Motivo del diferimiento:* a diferencia de §1 #4 (donde el `SqlException` real llega directo al catch), acá `ex` es una `Exception` genérica — la excepción real de SQL puede venir envuelta como `InnerException` desde múltiples orígenes distintos dentro del mismo `try` (cada uno con su propio `throw new Exception("...", ex)`, el mismo antipatrón de §10 Transversal #3). Este método es además el gate de integridad que corre ANTES del login: un fix apurado que camine mal la cadena de `InnerException` podría convertir un "toleramos, BD sin migrar" en un "bloqueamos el acceso" para un caso legítimo, o viceversa — haría falta reconstruir el árbol de excepciones envueltas con cuidado y sin poder testearlo (mismo problema de `SqlException` no instanciable de §1 #4). Se prefiere dejarlo explícitamente pendiente antes que tocar a las apuradas el candado de arranque del sistema.
...
8. ✅ RESUELTO (2026-09-14) — `GestorPermisos` sin resguardo visible contra que un usuario modifique/potencie el rol al que él mismo pertenece (auto-escalación de privilegios), a diferencia de la protección de "último administrador" que sí existe para usuarios puntuales.
*Solución aplicada:* `BLL.Familia.GuardarAsignacionRol` ya tenía anti-autobloqueo (no podés QUITARTE la gestión de tu propio rol) pero nada impedía AGREGARTE permisos que tu rol no resuelve hoy editando ese mismo rol. Se agregó el guard simétrico: se comparan las patentes efectivas actuales del rol propio contra las que resultarían de la selección propuesta (sin mutar el árbol real — cada id candidato se resuelve de forma independiente vía `ObtenerPatentesEfectivas()`) y se rechaza si la propuesta no es subconjunto de lo actual (`err.bll.familia.autoescalacion`). El núcleo de la decisión (`BLL.Familia.NoEscalaPrivilegios`) es un método estático puro, mismo estilo que `SistemaConservaGestion`, testeado de forma aislada (5 tests) más un test de integración del camino de rechazo (igual que el de autobloqueo existente, sin llegar a `ExigirSistemaConservaGestion` para no depender de una BD real). Alcance deliberadamente acotado a "editar el propio rol" (lo que pidió la auditoría) — un delegado con la patente de gestión sigue pudiendo ampliar OTROS roles, que es el comportamiento de delegación ya diseñado a propósito (Etapa 4), no el hallazgo.
...
*Solución aplicada:* se agregó `FormBase.Tr(clave, fallback, args = null)` (wrapper de `Traductor.Resolver` con el idioma activo) como único punto de traducción, y se migraron los ~121 usos encontrados (más de los ~84 estimados originalmente) en los 45 formularios que heredan `FormBase` — 14 commits, uno por lote, cada uno verificado con build + tests. Varios `RellenarCombo*`/`TraducirX` perdieron parámetros de idioma/diccionario que ya no necesitaban. Se preservaron intactos, a propósito, los dos closures locales de `MostrarEstadoActual()` en `CobroSuscripcionForm`/`RenovacionSuscripcionForm`, que cachean un único fetch del diccionario para 5 sub-resoluciones (optimización deliberada y documentada, no duplicación). *Alcance no cubierto (2026-09-10):* 11 formularios que heredan `Form` directo en vez de `FormBase` (`Login`, `Menu`, `MiPerfilForm`, `ConfirmarAdminForm`, `DesbloqueoEmergenciaForm`, `DiagnosticoIntegridadForm`, `ExploradorCompositeForm`, `RecuperacionEspejoForm`, `CambioClaveObligatorioForm`, `CargoPrendaDialog`, `CambioEstadoDialog`) no pueden usar el helper heredado sin además migrarlos a `FormBase` — cambio de mayor alcance, fuera de esta pasada. Los 3 `Exportadores` y `Program.cs` tampoco son `Form` y quedan igual sin tocar.

**✅ RESUELTO (2026-09-14)** — de los 11 de arriba, `RecuperacionEspejoForm` ya se había migrado en una sesión anterior (ver §10 Transversal #5). Los 8 restantes con motivo puramente histórico (no arquitectónico) se migraron ahora a `FormBase`: `ConfirmarAdminForm`, `DesbloqueoEmergenciaForm`, `CambioClaveObligatorioForm`, `CargoPrendaDialog`, `CambioEstadoDialog`, `MiPerfilForm`, `DiagnosticoIntegridadForm`, `ExploradorCompositeForm`. Cada uno perdió su try/catch de carga de `icon.ico` duplicado y su función de traducción local (`T`/`T_ce`/`T_dlg`, algunas redeclaradas más de una vez dentro del mismo archivo) a favor del `Tr()` heredado. Se preservaron intactos, a propósito, los dos mecanismos de traducción DISTINTOS que coexistían en dos de estos archivos (el patrón Tag+diccionario de `ConfirmarAdminForm.Traducir()`, y la resolución por nodo del árbol Composite en `ExploradorCompositeForm.CrearNodoRecursivo`) — no son la duplicación que se buscaba eliminar. `Login` y `Menu` quedan deliberadamente sin migrar: `Login` corre antes de que exista sesión (no hay usuario del cual cargar tema/fuente) y `Menu` es el contenedor MDI con su propia lógica de permisos y aplicación de preferencias. Trabajo hecho con dos agentes implementadores en paralelo (4 archivos cada uno) más un agente revisor independiente que releyó los 8 archivos completos, auditó específicamente un reemplazo con regex global usado en uno de ellos, y corrió el build — sin encontrar problemas bloqueantes (solo un `using System.Drawing;` sin uso en `CambioClaveObligatorioForm.cs`, corregido). Build + suite completa (426/428, mismos 2 skips de siempre) verificados.

**2. (Diferido) Convención de campos privados dividida por capa, no aleatoria.** DAL: 100% sin `_` (17 archivos). GUI: 100% con `_` (27 archivos, salvo `Login.cs`, única excepción). BLL: mezcla ambas **dentro de la misma clase** (11 archivos) según si el campo es inyectado (`dalPedido`) o lazy (`_listaEsperaLazy`) — regla informal nunca documentada. *Motivo del diferimiento:* renombrar campos privados en 17+ archivos de DAL (o unificar el criterio BLL) es un rename masivo de bajo riesgo pero alto ruido en el diff, sin bug asociado — mismo criterio que el resto de convenciones cosméticas ya diferidas (§7 Media #11, BLL field naming).

**3. (Diferido) DAL nunca usa la jerarquía de excepciones propia del proyecto.** 79 `throw new Exception(...)` genéricos en 14 archivos (12 en DAL) — pese a existir `BE.AppException`/`LoginException`/`SesionException`, usadas consistentemente en BLL/GUI para distinguir error esperado de inesperado (`GUI/FormBase.cs:113-118` chequea explícitamente `is BE.AppException`). *Motivo del diferimiento:* en la práctica no es un bug — la BLL ya envuelve/traduce estos errores como "inesperados" (`RegistrarExcepcion` + mensaje genérico), que es el comportamiento correcto para una falla real de DAL (SQL, conexión). Migrar 79 sitios a una jerarquía propia solo para distinguir tipos de error de infraestructura es un refactor grande sin beneficio de negocio claro — mismo criterio que el resto de refactors de DAL ya diferidos (§1 #4, #5, #6, #9).
...
**7. ✅ RESUELTO (2026-09-13) — Interfaces BLL: 17/17 con su única clase implementadora, 1 con nombre inconsistente** (`IRecomendacionService` debería ser `IRecomendacionPrendasService`).
*Solución aplicada:* renombrada a `IRecomendacionPrendasService` (archivo, csproj, clase implementadora `RecomendacionPrendas` y el campo tipado en `RecomendacionPrendasForm`). *Alcance no cubierto (diferido):* "17 clases BLL de nivel similar no implementan ninguna interfaz, sin una regla escrita que distinga cuáles la necesitan" queda como observación, no como defecto — la regla real y ya aplicada en todo el código es "tiene interfaz si necesita ser inyectada con un doble de test" (ver el propio Tests §9: se agregaron interfaces puntuales — `IAnalisisAbandonoService`, `IBitacoraService` — exactamente cuando una clase lo necesitó para ser testeable, no de forma preventiva). Escribir esa regla en ningún lado más allá de este párrafo no cambia comportamiento; no amerita una pasada adicional.

**8. (Diferido) Métodos más largos del repo (candidatos a god-method):** `Traductor.InferirFormulario` (~131 líneas), `Configuracion.VerificarIntegridadDV` (~129), `DashboardForm.ConstruirElementosCondicionales` (~110), `RecuperacionIntegridad.Diagnosticar` (~105), `Usuario.Autenticacion.Login` (~99), entre otros — mezclan validación, orquestación de DAL y side-effects en un solo bloque, a diferencia de los servicios que sí delegan en Manejadores/Estrategias. *Motivo del diferimiento:* son candidatos a extraer en sub-métodos/Manejadores, pero ninguno tiene un bug asociado ni falta de cobertura crítica detectada en esta auditoría — es trabajo de legibilidad, no de corrección, y partir 5 métodos de distintas capas es un lote de PRs por sí solo. Se deja la lista acá como punto de partida si se decide encararlo más adelante.
